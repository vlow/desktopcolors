...
`@astrojs/sitemap` (`astro.config.mjs`) as `sitemap-index.xml` + `sitemap-0.xml`, which the
`Sitemap:` line in `robots.txt` points at. No `filter` is configured and none is needed:
the integration lists page routes only, so the per-OS `view.json` endpoints stay out on
their own — 687 HTML pages in, 687 `<loc>` out. The e2e guard in `e2e/smoke.spec.ts` pins
that, since it is the integration's behaviour rather than ours.

**The sitemap version is pinned exactly** (`"@astrojs/sitemap": "3.2.1"`, no caret).
Versions from 3.3 on read the `routes` argument of `astro:build:done`, which exists only in
Astro 5, and fail this Astro 4 build outright with `Cannot read properties of undefined
(reading 'reduce')`. The package declares no `peerDependencies`, so npm resolves the broken
combination without a word — unpin it together with the Astro 5 upgrade, not before.

Both artefacts reach production through the ordinary `cp -R dist` in `deploy/rebuild.sh`;
there is no deploy-side step. Reasoning:
